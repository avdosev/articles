...
Мы полностью абстрагировались от представления данных и сконцентрировались лишь на содержимом. Благодаря этому мы в любой момент сможем с достаточной простотой обновить код преобразования (или рендер) json в html который мы отдаем в браузер. Только теперь мы сможем написать похожий рендер из jsonа не только в html, но и для нативных приложений без необходимости использовать Web View и аналоги.
...
1. Есть ли разница в скорости парсинга json с заинлайненым html и json с представленнием статьи в виде jsonа.
...
2. Производительности при двух проходах парсинга (сначала парсим json а потом html) примерно в 2 раза меньше и не в пользу html.
